...
function App() {
  ...
  const handleScan = async (blob) => {
    ...
    try {
      ...
      const result = { aiData, pricingData, imageBlob: blob };
      setScanResult(result);
      setLastScannedResult(result);
      setCurrentView('result');
    } catch (error) {
      console.error(error);
      setErrorData("Scan failed. Check Vercel Logs.");
    } finally {
  ...
  const Dashboard = () => (
    <div className="h-full flex flex-col p-6 overflow-y-auto">
      ...
      <div className="grid grid-cols-2 gap-4 auto-rows-[160px] flex-1">
        <div onClick={() => setCurrentView('scan')} className="col-span-2 glass-panel rounded-3xl p-6 relative overflow-hidden cursor-pointer group hover:border-neon-blue/50 transition-all duration-300 hover:scale-[1.02] active:scale-95">
          ...
          <div className="relative z-10 flex flex-col h-full justify-between">
            ...
            <div>
              ...
              <p className="text-xs text-gray-300">Identify comics instantly</p>
            </div>
          </div>
        </div>

        <div onClick={() => setCurrentView('manual')} className="glass-panel rounded-3xl p-5 relative overflow-hidden cursor-pointer group hover:border-neon-purple/50 transition-all duration-300 hover:scale-[1.02] active:scale-95 flex flex-col justify-between">
          ...
          <div><h3 className="text-lg font-bold text-white leading-tight">Manual<br />Lookup</h3></div>
        </div>

        <div onClick={() => lastScannedResult && setCurrentView('result')} className={`glass-panel rounded-3xl p-5 relative overflow-hidden flex flex-col justify-between transition-all duration-300 ${lastScannedResult ? 'cursor-pointer group hover:border-pink-500/50 hover:scale-[1.02] active:scale-95' : 'opacity-50 cursor-not-allowed'}`}>
          {lastScannedResult ? (
            <>
              <div className="absolute inset-0">
                {lastScannedResult.imageBlob && <img src={URL.createObjectURL(lastScannedResult.imageBlob)} className="w-full h-full object-cover opacity-30 group-hover:opacity-50 transition-opacity" alt="Last scanned" />}
            ...
            <>
              <div className="w-10 h-10 rounded-full bg-gray-700/50 flex items-center justify-center text-gray-500"><svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg></div>
              <div><h3 className="text-lg font-bold text-gray-500 leading-tight">No Recent<br />Scans</h3></div>
            </>
          )}
        </div>
      </div>
    ...
    <Layout currentView={currentView} onViewChange={setCurrentView}>
      {isProcessing && (
        <div className="absolute inset-0 bg-black/80 z-50 flex flex-col items-center justify-center p-4">
          ...
          <p className="text-white font-bold animate-pulse">Scanning Comic...</p>
        </div>
      )}
      {errorData && (
      ...
      {currentView === 'result' && <ResultCard data={scanResult} onRescan={() => setCurrentView('scan')} />}
    </Layout>
  );
}
