...
function baseExclusions() {
    // Keep this list editable and versioned
    return [
...
function extractCandidatePrices(browseJson) {
...
export async function priceComic({ seriesTitle, issueNumber }) {
    ...
    // 1) Browse active listings baseline (works even when sold scrape fails)
    const browse = await browseSearch({ q, limit: 50 });
    const { prices, imageUrl, count } = extractCandidatePrices(browse);
    const trimmed = trim(prices);

    let typical = percentile(trimmed, 0.50);
    let soft = percentile(trimmed, 0.25);

    // 2) Sold-comps fallback when browse is too thin/noisy
    // Heuristics: low count or empty typical
    let slabs = null;

    if (!typical || count < 10) {
        const cacheKey = `sold:${seriesTitle}:${issueNumber}`.toLowerCase();
        const sold = await soldComps({ query: `${seriesTitle} #${issueNumber} comic`, cacheKey });
